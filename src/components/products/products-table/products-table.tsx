...
export const ProductsTable = () => {
    const { stock, currentPage, pageSize, totalElements, isLoading, error, goToPage } = useStockContext();
    ...
    const columns: Column<StockResponse>[] = useMemo(() => [
...

        {
            header: 'Stock',
            render: (row) => (
                <span className={styles['stock-quantity']}>
                    {`${row.quantity} units`}
                </span>
            )
        },
