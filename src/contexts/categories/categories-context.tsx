...
export default function CategoryListProvider({ initialCategories, children }: { initialCategories: CategoryResponse[], children: React.ReactNode; }) {
    ...
    const handleAddCategory = async (category: CategoryRequest) => {
        ...
        try {
            ...
            setCategories(prev => prev.map(category => category.resourceId === optimisticResponse.resourceId ? response : category))
        } catch (fetchError) {
            setCategories(prev => prev.filter(category => category.resourceId !== optimisticResponse.resourceId))
    ...
    const handleUpdateCategory = async (resourceId: string, category: CategoryRequest) => {
        ...
        setCategories(prev => prev.map(category => category.resourceId === resourceId ? optimisticResponse : category))
        setError(null);
        ...
        try {
            ...
            setCategories(prev => prev.map(category => category.resourceId === resourceId ? currentCategory : category))
    ...
    const handleDeleteCategory = async (resourceId: string) => {
        const currentCategory = categories.find(category => category.resourceId === resourceId)!
        const currentCategoryIndex = categories.findIndex(category => category.resourceId === resourceId)
        ...
        try {
            ...
            setCategories(prev => [...prev.slice(0, currentCategoryIndex), currentCategory, ...prev.slice(currentCategoryIndex)])
    ...
    return (
        <CategoryContext.Provider value={{ categories, error, addCategory: handleAddCategory, updateCategory: handleUpdateCategory, deleteCategory: handleDeleteCategory }}>
            {children}
